...
let store = MongoStore.create(
  //some imp options for connect-mongo
  {
...
app.post("/verify", async (req, res) => {
  const { razorpay_order_id, razorpay_payment_id, razorpay_signature } =
    req.body; // we receives from res from handler function which is given by razorpay backend
  ...
  if (expectedSignature === razorpay_signature) {
    //if both signature matches then payment verifed
...
app.post("/wishlist/:listingId", isLoggedin, async (req, res) => {
  ...
  if (!user) {
    return res
      .status(401)
      .json({ message: "Unauthorized: User not found", redirect: "/login" });
  }
  ...
  if (!user.wishlist.includes(listingId)) {
    ...
    await sendNotification(
      ...
      "A new listing has been added to wishlist"
    );
  }
...
app.delete("/wishlist/:listingId", isLoggedin, async (req, res) => {
  ...
  if (user.wishlist.includes(listingId)) {
    ...
    await sendNotification(
      res.locals.currUser._id,
      "A listing has been removed from wishlist"
    );
  }
...
app.get("/notification", isLoggedin, async (req, res) => {
  ...
  await Notification.updateMany(
    { user: res.locals.currUser._id, isRead: false },
    { isRead: true }
  );
});
//notifoication count
